    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items-center mx-w-7xl mx-auto">
        ...
        <ul className="list-none hidden sm:flex flex-row gap-10">
          {
            navLinks.map((link)=>(
              <li key={link.id} className={`${active===link.title? "text-white" : "text-secondary"}
                   hover:text-white font-meduim cursor-pointer text[18px] `}
        ...
        <div className="sm:hidden flex flex-1 items-center justify-end">
          ...
          <div className={`${!toggle ? 'hidden' : 'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-width-[144px] z-10 rounded-xl`}> 
